...
describe('ApplyHours', () => {
  ...
  describe('toString', () => {
    it('ALL_DAY', () => {
      expect(new ApplyHours([0, 23]).toString()).toBe(text.ALL_DAY);
    });

    it('시간 배열을 텍스트로 변환', () => {
      const applyHours = [chance.hour(), chance.hour()];
      const result = new ApplyHours(applyHours).toString();

      expect(result).toContain(applyHours[0].toString());
      expect(result).toContain(applyHours[1].toString());
    });
  });
});
